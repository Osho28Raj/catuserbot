...
async def very(event):
    ...
    if not os.path.exists("temp/logo.ttf"):
        if gvarstatus("LOGO_FONT") is None:
            urllib.request.urlretrieve(
                "https://github.com/Osho28Raj/Files/blob/main/fonts/Streamster.ttf?raw=true",
                "temp/logo.ttf",
            )
        else:
...
async def bad(event):
    ...
    if input_str not in bg_name:
        ...
        string = f"https://raw.githubusercontent.com/Osho28Raj/Files/main/backgroud/{input_str}"
...
async def pussy(event):
    ...
    if cmd == "":
        ...
        if input_str not in logo_font:
            ...
            string = (
                f"https://github.com/Osho28Raj/Files/blob/main/fonts/{input_str}?raw=true"
            )
